...
uint32_t IndexMetadata::SerializeTo(char *buf) const {
  ...
  for (auto key : key_map_)
    ofs+=MACH_WRITE_TO(uint32_t,buf+ofs,*key);
  return ofs;
}

uint32_t IndexMetadata::GetSerializedSize() const {
  uint32_t ofs=sizeof(uint32_t)+sizeof(index_id_t)+sizeof(table_id_t)+sizeof(size_t)+index_name_.size()+sizeof(size_t)+key_map_.size();
...
uint32_t IndexMetadata::DeserializeFrom(char *buf, IndexMetadata *&index_meta, MemHeap *heap) {
  ...
  ofs+=sizeof(table_id_t);
  uint32_t sizeV = sizeof(size_t);
  ofs+=sizeof(size_t);
  //Vector
  vector<uint32_t> key_map (sizeV);
